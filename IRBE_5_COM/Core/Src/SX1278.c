...
void SX1278_hw_DelayUs(uint32_t usec) {
	  __HAL_TIM_SET_COUNTER(&htim3, 0);
	  __HAL_TIM_SET_AUTORELOAD(&htim3, 65000);
	  u_sec_delay = 0;
	  HAL_TIM_Base_Start(&htim3);
		while(1){
			if(__HAL_TIM_GET_COUNTER(&htim3) >= usec){
				break;
			}
		}
...
uint8_t SX1278_RSSI_PACKET_LoRa(SX1278_t * module) {
	uint32_t temp = 10;
	temp = SX1278_SPIRead(module, LR_RegPktRssiValue); //Read RegRssiValue, Rssi value
	temp = 164 - temp; //127:Max RSSI, 137:RSSI offset
	return (uint8_t) temp;
}
